...
const db = {
  ...
  getUserSessions: (userId, from, to) => {
    let sql = `SELECT s.*, o.name as object_name FROM sessions s JOIN objects o ON s.object_id = o.id WHERE s.user_id = ?`;
    const params = [userId];
  ...
  getAllEmployees: () => queryAll(`SELECT u.id, u.name, u.email, u.created_at, (SELECT o.name FROM sessions s JOIN objects o ON s.object_id = o.id WHERE s.user_id = u.id AND s.status = 'active' LIMIT 1) as current_object, (SELECT COUNT(*) FROM sessions WHERE user_id = u.id) as total_sessions FROM users u WHERE u.role = 'employee' ORDER BY u.name`),
  getAllActiveSessions: () => queryAll(`SELECT s.id, s.arrived_at, u.name as user_name, u.id as user_id, o.name as object_name FROM sessions s JOIN users u ON s.user_id = u.id JOIN objects o ON s.object_id = o.id WHERE s.status = 'active' ORDER BY s.arrived_at DESC`),
};
